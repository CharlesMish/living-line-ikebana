...
Every hydrated or appended record is reconstructed from an explicit field allowlist (never a pass-through of parsed JSON, so tampered/undeclared fields never survive) and checked against semantic rules — a hit always has an operation; camera resolves only `released`/`cancelled`; a graph edit resolves only `committed`/`cancelled`; only an invalid insertion resolves `declined`; `insert` always carries both `materialId` and `inputMethod`, nothing else carries either; a cancellation always carries a reason; every timing value is finite and nonnegative. Anything that fails is dropped, never partially trusted.

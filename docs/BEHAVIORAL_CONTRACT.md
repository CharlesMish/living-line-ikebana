...
- Optional `?experiment=bend-stations` does not revise this default. It stays off unless that exact flag is present and touch bend is not active. While it is on, one bead uses a chosen legal station among experimental fractions `0.32`, `0.54`, and `0.76`, collapsed when clamping makes them the same distance. The choice is temporary UI state. Touch bend is unchanged and is never combined with this flag. See `docs/development/REVIEW_bend-stations.md`.
...
- While `?experiment=bend-stations` is active, new acquisitions are not appended to either study bucket and are not treated as ordinary fixed-bead observations. Existing study data is left in place. Transaction completion, cancellation, and committed graph saves still run. A session where the flag was requested but touch bend excluded it keeps recording on the touch arm.

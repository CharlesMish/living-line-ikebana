...
| Cutting | Structure | Appearance | Bend response |
| --- | --- | --- | --- |
| Foliage fan / `foliage-fan-v1` | One stem, three lateral arms, eight small leaves on those arms. A basal span stays below the first arm | Yellow-green stock `0x7c9a34`. Elliptic leaves use the `spray` draw, color `0x3e6b38`, vein `0xd5e6a6`, roughness `0.74`, still smaller than the leafy shoot's blades. `?fanLeaf=shared` and `?fanLeaf=separated` recover the other outlines without rewriting the graph | Stem `0.47`, arms `0.36`, stalks `0.18`, copied once. Shared solver |
...
separate organs. The accepted outline is `spray` on the elliptic grid; the graph
stays `foliage-fan-v1`. The blossom spray uses the
...
| Fern frond / `fern-frond-v1` | One 16-segment rachis and eight alternating pinna stalks. Each stalk carries one divided pinna, not a swarm of pinnules. A basal span stays below the first pinna | Rachis `0x2c5c45`, roughness `0.72`. Pinnae use the `pinnate` surface with the accepted `tapered` draw: a costa plus three pointed pairs, color `0x7eae62`. `?pinnate=baseline` and `?pinnate=quilled` recover the other draws. Hit radius `0.78` centered at local Y `0.56`. No bloom is generated | Rachis `0.43`, stalks `0.18`, copied once. Stalks are petioles, so the shared solver bends the rachis only |

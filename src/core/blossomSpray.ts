...
export const BLOSSOM_SPRAY_VERSION = "blossom-spray-v1" as const;
 ...
 */
export function createBlossomSpray(id: string, seed: number, base: Vec3): PlantGraph {
  ...
  const length = 5.15 + random.next() * 0.2;
  ...
  flowerCounts.forEach((flowerCount, groupIndex) => {
    ...
    const direction = directionFromParent(anchor.tangent, spin, 0.9, 0.34);
    ...
    const lateral = addBranch(graph, {
      ...
      points: makeChain(
        ...
        8,
        direction,
        add(scale(side, 0.05), vec3(0, -0.035, 0)),
      ),
      radius: 0.015,
    ...
    for (let flowerIndex = 0; flowerIndex < flowerCount; flowerIndex += 1) {
      ...
      const stalkDirection = directionFromParent(stalkAnchor.tangent, stalkSpin, 0.94, 0.16);
      ...
      const pedicel = addBranch(graph, {
        ...
        points: makeChain(
          ...
          0.5 + random.next() * 0.06,
          4,
          stalkDirection,
          add(scale(stalkSide, 0.03), vec3(0, -0.05, 0)),
        ),
        radius: 0.011,

...
import { NODDING_FLOWER_RESPONSE } from "./materialResponse.ts";
import { Mulberry32 } from "./prng.ts";
import { SCHEMA_VERSION, type PlantGraph } from "./types.ts";

export const NODDING_FLOWER_VERSION = "nodding-flower-v1" as const;
...
/**
 * One bell on a curved neck, above a short accessible stem and a single leaf.
 * The nod is rest geometry: the neck is sampled once and then edited only by
 * the shared aim, bend, and prune laws. It is not a gravity or droop simulation.
 * Generic, not a species. Randomness is consumed only during construction.
 */
export function createNoddingFlower(id: string, seed: number, base: Vec3): PlantGraph {
  ...
  const stem = addBranch(graph, {
    ...
    radius: 0.031,
    stiffness: NODDING_FLOWER_RESPONSE.stem,
  });
  ...
  const petiole = addBranch(graph, {
    ...
    radius: 0.012,
    stiffness: NODDING_FLOWER_RESPONSE.stalk,
  });
  ...
  const neckDistance = stem.activeLength * 0.9;
  const neckAnchor = sampleBranch(stem, neckDistance);
  const rise = scale(neckAnchor.tangent, 0.36);
  const crest = add(
    neckAnchor.position,

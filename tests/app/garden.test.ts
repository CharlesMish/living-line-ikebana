...
test("workbench fixtures are reproducible, valid and registered for every reference/seed/count", () => {
  for (const material of ["flowering-branch", "leafy-shoot", "bare-branch", "single-flower", "arching-trailer", "mixed", "reference-pair"]) for (const seed of WORKBENCH_SEEDS) for (const count of [1, 2, 6, 12]) {

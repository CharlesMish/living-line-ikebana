...
test("registered material appearances rebuild consistently and leafy hit proxies cover their blades", async () => {
  ...
  const studio = Object.assign(Object.create(ThreeStudio.prototype), { options: { debugHitTargets: false } });
  const stemColors = [];
  for (const material of ["flowering-branch", "leafy-shoot", "bare-branch", "single-flower", "flower-volume"]) {
  ...
  assert.notEqual(stemColors[0], stemColors[4], "flower-volume stem must not inherit woody trunk appearance");
  assert.notEqual(stemColors[1], stemColors[4], "flower-volume stem must remain distinct from leafy shoot");
  assert.notEqual(stemColors[3], stemColors[4], "flower-volume stem must remain distinct from the single flower");
});

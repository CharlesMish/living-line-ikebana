...
test("both material appearances rebuild consistently and leafy hit proxies cover their blades", async () => {
  ...
  const studio = Object.assign(Object.create(ThreeStudio.prototype), { options: { debugHitTargets: false } });
  const stemColors = [];
  for (const material of ["flowering-branch", "leafy-shoot", "single-flower"]) {
  ...
  assert.notEqual(stemColors[0], stemColors[2], "single-flower stem must not inherit woody trunk appearance");
  assert.notEqual(stemColors[1], stemColors[2], "single-flower stem must remain distinct from leafy shoot");
});

...
test("registered material appearances rebuild consistently and leafy hit proxies cover their blades", async () => {
  ...
  const studio = Object.assign(Object.create(ThreeStudio.prototype), { options: { debugHitTargets: false } });
  const stemColors = [];
  for (const material of ["flowering-branch", "leafy-shoot", "bare-branch", "single-flower", "reed", "flower-volume", "arching-trailer", "nodding-flower"]) {
  ...
  assert.notEqual(stemColors[7], stemColors[3], "nodding flower must not inherit the single-flower stem");
  assert.notEqual(stemColors[7], stemColors[5], "nodding flower must not inherit the flower-volume stem");
  assert.notEqual(stemColors[7], stemColors[0], "nodding flower must not inherit woody trunk appearance");
});
